...
export default function Portfolio({ projects }) {
  ...
  const hrefProject = (slug) => `/?p=${slug}`;
  const hrefImage = (slug, i) => `/?p=${slug}&img=${i}`;
    ...
    <div className="min-h-screen bg-white text-black flex flex-col items-center">
      ...
      {selectedProject && selectedProject.name !== "Info" ? (
        <div className="w-full max-w-6xl px-6 py-12">
          {/* Descrizione */}
          <p className="text-sm md:text-base leading-relaxed text-black mb-10 max-w-3xl whitespace-pre-line">
      ...
      <style jsx>{`
        ...
        .marquee-track {
          ...
          will-change: transform; animation: marquee 80s linear infinite;
        }
        .marquee-track.reverse { animation-direction: reverse; }
        .marquee-seq { display: inline-block; padding-right: 2rem; font-size: 7rem; font-weight: 800; text-transform: uppercase; cursor: pointer; }
        @keyframes marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }

        /* Ken Burns OUT leggermente più evidente (su entrambi i layer) */
        @keyframes kbOut {
          0%   { transform: scale(1.03); }
          100% { transform: scale(1.00); }
        }
        .animate-kb-out {
          animation: kbOut 12s ease-in-out both;
        }
        ...
        .fade-in  { animation: fadeIn 1600ms ease-in-out both; }
        .fade-out { animation: fadeOut 1600ms ease-in-out both; }
      `}</style>
    </div>
  );
}
